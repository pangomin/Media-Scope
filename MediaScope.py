...
class TelegramMediaAnalyzer:
    ...
    def _get_media_type(self, message) -> Optional[str]:
        ...
        if hasattr(message.media, 'document'):
            ...
            # If we couldn't determine the type but have a document, return 'other'
            return 'other'
                ...
                *Progress.get_default_columns(),
                ...
                async for message in self.client.iter_messages(channel):
                    if hasattr(message.media, 'document'):
                        ...
                        self.stats['media_types'][media_type] = self.stats['media_types'].get(media_type, 0) + 1
    ...
    async def _display_results(self, channel_title: str) -> None:
        ...
        if self.stats['media_types']:
            ...
            for media_type, count in sorted(self.stats['media_types'].items()):
                percentage = (count / self.stats['file_count']) * 100
                type_table.add_row(
                    media_type.capitalize() if media_type else 'Unknown',
                    str(count),
                    self._format_size(self.stats['total_size'] * (percentage / 100)),
                    f"{percentage:.1f}%"
                )
